...
def open_and_login(driver,login,pswd):
    status = True
    if driver == 'firefox':
    ...
    if status:
        ...
        driver.find_element_by_xpath('//*[@id="page-container"]/div/div[1]/form/fieldset/div[2]/input').send_keys(pswd)
        ...
        print("please provide a valid driver. Either firefox or Chrome")
...
def capture_ẗweet_data(tweets):
    ...
    for index, tweet in enumerate(tweets):
        ...
        user.append(tweet.select_one("div[class='css-1dbjc4n r-18u37iz r-dnmrzs']").text)
    ...
    kle = pd.DataFrame(list(zip(user, datetime, tweet_text, comments, retweets, likes, tweet_url)), columns = ["sender", "datetime", "text", "comments", "retweets", "likes", "tweet_url"])
    return kle
...
def collect_full_timeline(driver, testing=False, test_runs=3, return_cleaned=True, write_csv=True):
    ...
    if write_csv:
        ...
        twets.to_csv("timeline_" + today + ".csv")
